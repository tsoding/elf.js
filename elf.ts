...
const ELFOSABI_LABELS: {[key: string]: string} = {
    '0': 'SYSV',
    '1': 'HPUX',
    '2': 'NETBSD',
    '3': 'LINUX',
...
const sizeOfType: { [key: string]: number } = {
    'u8': 1,
    'u16': 2,
    'u32': 4,
    'u64': 8,
...
type Scheme = Array<[string, string]>
type Struct = { [key: string]: number }

const Elf64_Ehdr: Scheme = [
    ['half', 'e_type'],
    ['half', 'e_machine'],
    ['word', 'e_version'],
    ['addr', 'e_entry'],
...
function serializeStruct(scheme: Scheme, struct: Struct, endian: Endian) {
    ...
    for (let i = 0; i < scheme.length; ++i) {
        ...
        switch (endian) {
            case 'LSB':
                if (size == 8) result.writeBigUInt64LE(BigInt(value), bytesIndex);
                else result.writeUIntLE(value, bytesIndex, size);
                break;
            case 'MSB':
                if (size == 8) result.writeBigUInt64BE(BigInt(value), bytesIndex);
                else result.writeUIntBE(value, bytesIndex, size);
                break;
            default:
                throw 'unreachable';
        }
...
function parseStruct(scheme: Scheme, bytes: Buffer, endian: Endian): Struct {
    ...
    for (let i = 0; i < scheme.length; ++i) {
        ...
        switch (endian) {
            case 'LSB':
                if (size == 8) {
                    const x = Number(bytes.readBigUInt64LE(bytesIndex));
                    console.assert(Number.isSafeInteger(x));
                    result[field] = x;
                } else result[field] = bytes.readUIntLE(bytesIndex, size);
                break;
            case 'MSB':
                if (size == 8) {
                    const x = Number(bytes.readBigUInt64BE(bytesIndex));
                    console.assert(Number.isSafeInteger(x));
                    result[field] = x;
                }
                else result[field] = bytes.readUIntBE(bytesIndex, size);
                break;
...
function sizeOfStruct(scheme: Scheme) {
    let size = 0;
    for (let i = 0; i < scheme.length; ++i) {
...
function verifyElfMag(bytes: Buffer) {
    for (let i = 0; i < 4; ++i) {
        if (bytes[i] != ELFMAG[i]) {
            return false;
        }
...
function getPhdrByIndex(fd: number, ehdr: Struct, index: number) {
...
type Reg = 0xC0 | 0xC7 | 0xC6 | 0xC2;
const RAX: Reg = 0xC0;
const RDI: Reg = 0xC7;
const RSI: Reg = 0xC6;
const RDX: Reg = 0xC2;

function syscall(): Buffer {
    return Buffer.from([0x0f, 0x05]);
}

function mov(reg: Reg, value: number): Buffer {
...
function write(hello: Addr, hello_sz: number) {
...
function helloWorldProgram(hello: Addr, hello_sz: number) {
    return Buffer.concat([
...
const subcmds: {[key: string]: Subcmd} = {
    'parse': {
        ...
        'run': (args) => {
            ...
            if (name) {
                ...
                for (let name in subcmds) {
                    console.error(`    ${name} ${subcmds[name].args} - ${subcmds[name].description}`);
                }
            }
        }
    },
...
function parseExecutable(argv: Array<string>) {
...
function generateExecutable(args: Array<string>) {
